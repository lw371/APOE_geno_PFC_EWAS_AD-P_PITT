...
runEWAS <- function(row, QCmetrics_2_BS) {
  APOE_2 <- lm(row ~ QCmetrics_2_BS$Number_of_2_alleles + QCmetrics_2_BS$Sex + 
                 QCmetrics_2_BS$Age + QCmetrics_2_BS$PMI + QCmetrics_2_BS$Braak_Stage_AD)
  
  # Extract coefficients, std.error, and p-value
  results <- c(summary(APOE_2)$coefficients["QCmetrics_2_BS$Number_of_2_alleles1", c(1,2,4)])
...
QCmetrics_2_BS$Number_of_2_alleles <- factor(QCmetrics_2_BS$Number_of_2_alleles, levels = c("0", "1"))

# Ensure levels exist
print(levels(QCmetrics_2_BS$Number_of_2_alleles))

# Subset beta matrix
normbeta <- normbeta[, QCmetrics_2_BS$Basename, drop = FALSE]
...
clusterExport(cl, list("runEWAS", "QCmetrics_2_BS"))

# Run regression in parallel
outtab <- matrix(data = parApply(cl, normbeta, 1, runEWAS, QCmetrics_2_BS), ncol = 3, byrow = TRUE)
...
# Colnames
modTerms <- c("QCmetrics_2_BS$Number_of_2_alleles1")
...
filePath <- file.path(projDir, "4_analysis/results", paste0("APOE_2_with_Braak", "_EWASout.rdat"))
