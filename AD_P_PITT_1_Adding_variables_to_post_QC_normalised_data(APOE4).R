...
QCmetrics_4_BS <- merge(QCmetrics, ADD_VARS, 
                        by.x = colnames(QCmetrics)[3],  
                        by.y = colnames(ADD_VARS)[1],  
                        all.x = TRUE)

## Remove rows with missing values in 'Number_of_4_alleles'
QCmetrics_4_BS <- QCmetrics_4_BS[!is.na(QCmetrics_4_BS$Number_of_4_alleles), ]

## Create an additional copy for EWAS without Braak stage as a cofactor 
QCmetrics_4_null <- QCmetrics_4_BS

## Save QCmetrics_4_BS into normalised.rdata alongside existing objects
